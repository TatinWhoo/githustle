...
// Weights sum to 100. This is intentionally plain, readable business
// logic in the service layer — not hidden in a DB trigger — because
// "what counts as a complete profile" is a product decision that
// changes over time, and product decisions belong in application code
// where they're easy to find and easy to change.
...
// display_name is required at creation time, so it's worth the
// remaining 10 points unconditionally once a profile exists at all.
...
// Recomputes and persists the completion score. Called after every
// mutation that could affect it (profile edit, skill change, portfolio
// change) — recalculating fresh from the database each time, rather
// than trying to incrementally patch a running total, is the
// trustworthy approach: it can never drift from what's actually stored.
...
async function createFreelancerProfile(userId, data) {
  ...
  if (existing) {
    throw new AppError('Freelancer profile already exists for this account.', 409);
  }
...
async function getOwnFreelancerProfile(userId) {
  ...
  if (!profile) {
    throw new AppError('No freelancer profile found for this account yet.', 404);
  }
...
async function searchFreelancers(filters) {
  ...
  // We fetched `limit + 1` rows on purpose (see the repository comment)
  // — if that extra row exists, trim it off and report hasMore: true.
...
async function createClientProfile(userId, data) {
  ...
  if (existing) {
    throw new AppError('Client profile already exists for this account.', 409);
  }
...
async function getOwnClientProfile(userId) {
  ...
  if (!profile) {
    throw new AppError('No client profile found for this account yet.', 404);
  }
  return toClientDTO(profile);
}
...
async function replaceFreelancerSkills(userId, skills) {
  const profile = await repo.findFreelancerProfileByUserId(userId);
  if (!profile) {
    throw new AppError('Create your freelancer profile before adding skills.', 404);
  }
...
// Ownership check shared by update and delete — confirms the portfolio
// item being mutated actually belongs to the requesting user's own
// freelancer profile. This is the ABAC half of authorization: RBAC
// (requireRole middleware) confirms "you're A freelancer"; this
// confirms "you're THIS specific resource's freelancer."
async function assertOwnsPortfolioItem(userId, itemId) {
  const profile = await repo.findFreelancerProfileByUserId(userId);
  if (!profile) {
    throw new AppError('No freelancer profile found for this account.', 404);
  }
  ...
  if (!item || item.freelancer_profile_id !== profile.id) {
    ...
    // else — a 403 here would confirm to an attacker that the ID is
    // valid but just owned by another user.
...
async function addPortfolioItem(userId, data) {
  const profile = await repo.findFreelancerProfileByUserId(userId);
  if (!profile) {
    throw new AppError('Create your freelancer profile before adding portfolio items.', 404);
  }
...
// ── Uploads ───────────────────────────────────────────────────
// Shared by both avatar endpoints (freelancer + client) and the
// portfolio image endpoint. This is the full pipeline: validate the
// file is genuinely what it claims to be, persist it, record it, and
// hand back a URL.
...
async function processImageUpload({ buffer, declaredMimeType, originalName, subdir, uploaderId, entityType }) {
  if (!isValidImageBuffer(buffer, declaredMimeType)) {
    throw new AppError(
      'File content does not match a valid image format. The file may be corrupted or mislabeled.',
      422
    );
  }
...
async function uploadFreelancerAvatar(userId, file) {
  const existing = await repo.findFreelancerProfileByUserId(userId);
  if (!existing) {
    throw new AppError('Create your freelancer profile before uploading an avatar.', 404);
  }
  ...
  // Clean up the old file from disk AFTER the new one is safely saved
  // — never delete-then-save, or a failure mid-upload leaves the user
  // with no avatar at all instead of just an outdated one.
...
async function uploadClientAvatar(userId, file) {
  const existing = await repo.findClientProfileByUserId(userId);
  if (!existing) {
    throw new AppError('Create your client profile before uploading an avatar.', 404);
  }
...
// Portfolio image upload is decoupled from a specific portfolio item:
// it just stores the file and returns a URL. The frontend then includes
// that URL in the create/update portfolio item request body. This
// avoids forcing an upload-before-the-item-exists ordering problem.
async function uploadPortfolioImage(userId, file) {
  const existing = await repo.findFreelancerProfileByUserId(userId);
  if (!existing) {
    throw new AppError('Create your freelancer profile before uploading images.', 404);
  }
