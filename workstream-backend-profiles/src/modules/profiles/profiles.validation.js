...
// Same shape, but every field optional — used for PUT (partial update).
...
// ── Skill tagging ─────────────────────────────────────────────
// The client sends the FULL desired skill set on every call; the
// service replaces what's stored with exactly this set inside a
// transaction. This is simpler and less error-prone for the frontend
// than separate add/remove-one-at-a-time endpoints — there's no way for
// the UI and DB to drift out of sync mid-edit.
...
// ── Search / browse freelancers ────────────────────────────────

const searchFreelancersQuerySchema = z.object({
  q: z.string().trim().max(200).optional(),
  skill: z.string().uuid().optional(), // skillId
