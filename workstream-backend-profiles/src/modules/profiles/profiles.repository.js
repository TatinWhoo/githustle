...
// All raw SQL for profiles, skills, and portfolio items lives here.
// Services never write SQL directly — see the same reasoning in
// auth.repository.js from Milestone 1.
...
// Builds an UPDATE ... SET clause from only the fields actually present
// in `data`, so a partial PUT body never overwrites untouched columns
// with NULL. This pattern shows up constantly in real CRUD APIs.
async function updateFreelancerProfile(userId, data) {
  ...
  if (setClauses.length === 0) {
    return findFreelancerProfileByUserId(userId);
  }
...
// Search/browse with optional full-text query, skill filter, rate
// range, availability, and location — built as parameterized fragments,
// never string-interpolated values, so this stays immune to SQL
// injection no matter how many filters get added later.
async function searchFreelancers(filters) {
  ...
  if (filters.skill) {
    // Correlated EXISTS subquery — uses idx_freelancer_skills_skill_id
    // (added in this milestone's migration) to avoid scanning the
    // entire junction table per candidate freelancer.
    conditions.push(
  ...
  if (filters.minRate !== undefined) {
    conditions.push(`fp.hourly_rate >= $${i}`);
  ...
  if (filters.available !== undefined) {
    conditions.push(`fp.is_available = $${i}`);
    values.push(filters.available);
    i += 1;
  }

  if (filters.location) {
    conditions.push(`fp.location ILIKE $${i}`);
  ...
  // Fetch one extra row beyond the page size — if it comes back, we
  // know there's a next page, without running a separate COUNT(*)
  // query. COUNT(*) over a filtered, growing table is exactly the kind
  // of query that gets slow first; this sidesteps it entirely.
...
async function updateClientProfile(userId, data) {
  ...
  if (setClauses.length === 0) {
    return findClientProfileByUserId(userId);
  }
...
// ── Skills (admin-managed lookup table) ──────────────────────
...
// Confirms every skillId the client sent actually exists before we
// commit to writing them — without this, a typo'd or fabricated UUID
// would silently insert a junction row pointing at nothing meaningful
// (the FK constraint would catch a truly invalid UUID, but a
// well-formed UUID for a skill that was deactivated would slip through).
...
// Replaces the ENTIRE skill set for a freelancer profile atomically.
// "Delete everything, then insert the new set" is only safe wrapped in
// a transaction — if the insert half failed without the transaction,
// the freelancer would be left with zero skills, not their original
// set. withTransaction() guarantees both halves succeed or neither does.
...
async function updatePortfolioItem(id, data) {
  ...
  if (setClauses.length === 0) {
    return findPortfolioItemById(id);
  }
...
// ── File uploads (shared metadata table) ─────────────────────

async function insertFileUpload({
  uploaderId,
  fileName,
  originalName,
  fileUrl,
