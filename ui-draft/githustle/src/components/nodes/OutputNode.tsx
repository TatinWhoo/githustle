...
export default memo(function OutputNode({ id, data, selected }: NodeProps) {
  ...
  const nodeColor = nodeData.color || '#0EA5E9';
    ...
    <div className="relative select-none w-full h-full" style={{ minWidth: 140, minHeight: 48, opacity: (nodeData.opacity ?? 100) / 100, zIndex: selected ? 1000 : undefined }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 z-10">
        <input
          ...
          style={{ fontSize: `${nodeData.fontSize || 11}px`, color: nodeData.textColor || '#0284C7' }}
        ...
        {!context?.isPersonal && nodeData.assignedRole && !selected && (
          <span className="text-[7px] font-mono text-sky-800 bg-sky-100/60 px-1 py-0.5 rounded border border-sky-200 uppercase font-bold mt-0.5">
      ...
      {/* 4-point handles */}
      <Handle type="target" position={Position.Top}    id="t-top"    className="!bg-sky-600 !border-white !w-3 !h-3" />
      <Handle type="source" position={Position.Top}    id="s-top"    className="!bg-sky-600 !border-white !w-3 !h-3" />
      <Handle type="target" position={Position.Bottom} id="t-bottom" className="!bg-sky-600 !border-white !w-3 !h-3" />
      <Handle type="source" position={Position.Bottom} id="s-bottom" className="!bg-sky-600 !border-white !w-3 !h-3" />
      <Handle type="target" position={Position.Left}   id="t-left"   className="!bg-sky-600 !border-white !w-3 !h-3" />
      <Handle type="source" position={Position.Left}   id="s-left"   className="!bg-sky-600 !border-white !w-3 !h-3" />
      <Handle type="target" position={Position.Right}  id="t-right"  className="!bg-sky-600 !border-white !w-3 !h-3" />
      <Handle type="source" position={Position.Right}  id="s-right"  className="!bg-sky-600 !border-white !w-3 !h-3" />
    </div>
  );
});
