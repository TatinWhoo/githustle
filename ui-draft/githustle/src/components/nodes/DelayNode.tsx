...
export default memo(function DelayNode({ id, data, selected }: NodeProps) {
  ...
  const nodeColor = nodeData.color || '#EC4899';
  ...
  return (
    <div
      ...
      className={`border flex flex-col items-center justify-center text-center px-3 py-2 shadow-sm relative transition-shadow select-none ${
    ...
    >
      ...
      <input
        ...
        style={{ fontSize: `${nodeData.fontSize || 11}px`, color: nodeData.textColor || '#BE185D' }}
      ...
      {!context?.isPersonal && nodeData.assignedRole && !selected && (
        <span className="text-[7px] font-mono text-pink-800 bg-pink-100/60 px-1 py-0.5 rounded border border-pink-200 uppercase font-bold mt-0.5">
          {nodeData.assignedRole}
        </span>
      )}

      {/* 4-point handles */}
      <Handle type="target" position={Position.Top}    id="t-top"    className="!bg-pink-600 !border-white !w-3 !h-3" />
      <Handle type="source" position={Position.Top}    id="s-top"    className="!bg-pink-600 !border-white !w-3 !h-3" />
      <Handle type="target" position={Position.Bottom} id="t-bottom" className="!bg-pink-600 !border-white !w-3 !h-3" />
      <Handle type="source" position={Position.Bottom} id="s-bottom" className="!bg-pink-600 !border-white !w-3 !h-3" />
      <Handle type="target" position={Position.Left}   id="t-left"   className="!bg-pink-600 !border-white !w-3 !h-3" />
      <Handle type="source" position={Position.Left}   id="s-left"   className="!bg-pink-600 !border-white !w-3 !h-3" />
      <Handle type="target" position={Position.Right}  id="t-right"  className="!bg-pink-600 !border-white !w-3 !h-3" />
      <Handle type="source" position={Position.Right}  id="s-right"  className="!bg-pink-600 !border-white !w-3 !h-3" />
    </div>
  );
});
